...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches || net->max_batches == 0){
        ...
        //}
        if((get_current_batch(net) < 1000 && get_current_batch(net) % 100 == 0) || get_current_batch(net) % 1000 == 0){
            char buff[256];
...
void test_classifier(char *datacfg, char *cfgfile, char *weightfile, int target_layer)
{
    ...
    char *test_list = option_find_str(options, "test", "data/darknet12_4-valid.list");
    int classes = option_find_int(options, "classes", 2);
    ...
    int n_car = 0;
    int n_bg = 0;
    FILE *fp = fopen("../darknet_v3/wrong.txt", "w");
    for(curr = net->batch; curr < m; curr += net->batch){
        ...
        if (curr<5358){
            if (pred.vals[0][0] > pred.vals[0][1]) n_car++;
            else {
                fputs(paths[curr-net->batch], fp); 
                fputs("\n", fp);
            }
        }
        else{
            if (pred.vals[0][0] < pred.vals[0][1]) n_bg++;
            else {
                fputs(paths[curr-net->batch], fp); 
                fputs("\n", fp);
            }
    ...
    fprintf(stderr, "%d %d\n", n_car, n_bg);
    float rc_car = (float)n_car/5357;
    float rc_bg = (float)n_bg/1918;
    float pr_car = (float)n_car/(n_car+1918-n_bg);
    float f1 = (float)2*rc_car*pr_car/(rc_car+pr_car);
    fprintf(stderr, "The recall(car) is %f, the specificity(bg) is %f, the precision(car) is %f, the f1 is %f\n", rc_car, rc_bg, pr_car, f1);
    fclose(fp);
}
